...
internal class SolutionTest {
    ...
    fun findPoisonedDuration() {
        ...
        assertEquals(2,solution.findPoisonedDuration(intArrayOf(1),2))
        assertEquals(0,solution.findPoisonedDuration(intArrayOf(),2))
    }
}
class Solution {
    fun findPoisonedDuration(timeSeries: IntArray, duration: Int): Int {
        ...
        for(item in timeSeries){
            if(prev == -1){
                ...
                count += minOf(item-prev, duration)
                prev = item
            }
        }
